...
function YAxis(props) {
    ...
    return (
        <g className="axis axis--y">
            {/* vertical line starting from the top (y1={0}) to the bottom (y2={height}) of the axis container.
            maps over the domain of the yScale and renders ticks and labels for each value. */}
            ...
            {yScale.domain().map((tickValue) => (
                <g key={tickValue} transform={`translate(0, ${yScale(tickValue) + yScale.bandwidth() / 2})`}>
                    ...
                    {/* Display the value. The x position of the text is calculated as -offsetX + 10, where offsetX is a prop passed to the component. */}
                    <text
